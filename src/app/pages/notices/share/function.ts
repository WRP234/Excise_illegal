...
export class ShareFunctions extends variable implements OnDestroy {
  ...
  onNoticeDateChange(event: IMyDateModel) {
    ...
    this._noticeDate.jsdate.setDate(
      this._noticeDate.jsdate.getDate() +
        parseInt(noticeDue) -
        (parseInt(noticeDue) < 1 ? 0 : 1)
    );
  ...
  checkDate() {
    const _sdate = this._noticeDate
      ? this._noticeDate
      : this.noticeForm.value.NoticeDate;
    const _edate = this._noticeDueDate
      ? this._noticeDueDate
      : this.noticeForm.value.NoticeDueDate;
      ...
      .map((term) =>
        term === ""
          ...
          : this.typeheadProduct
              .filter(
                (v) =>
                  v.ProductDesc.toLowerCase().indexOf(term.toLowerCase()) > -1
              )
              .slice(0, 10)
      );
      ...
      .map((term) =>
        term === ""
          ...
          : this.typeheadProductUnit
              .filter(
                (v) => v.DutyCode.toLowerCase().indexOf(term.toLowerCase()) > -1
              )
              .slice(0, 10)
      );
  ...
  formatterUnit = (x: { QUANTITY_UNIT: String }) => `${x.QUANTITY_UNIT || ""}`;
  ...
  public addNoticeDueDate(e: any) {
    ...
    if (/^0/.test(e.value))
      this.noticeForm.controls["NOTICE_DUE"].setValue(
        e.value.replace(/^0/, "")
      );
    ...
    if (!this.noticeForm.value.NOTICE_DATE) {
      this.noticeForm.patchValue({
        ...
        NOTICE_TIME: `${setZero(new Date().getHours())}:${setZero(
          new Date().getMinutes()
        )}`,
      });
    }
    ...
    noticeDate.setDate(
      noticeDate.getDate() + parseInt(dueDate) - (parseInt(dueDate) < 1 ? 0 : 1)
    );
    ...
    switch (type) {
      case "LS":
        if (event.getValue())
          this.notice.sort((a, b) => {
            return (
              <number>parseInt(b.NOTICE_CODE.substring(8)) -
              <number>parseInt(a.NOTICE_CODE.substring(8))
            );
          });
        else
          this.notice.sort((a, b) => {
      ...
      case "TIME":
        if (event.getValue())
          this.notice.sort((a, b) => {
            return (
              <any>new Date(b.NOTICE_DATE).getTime() -
              <any>new Date(a.NOTICE_DATE).getTime()
            );
          });
        else
          this.notice.sort((a, b) => {
  ...
  public setProductName(PRODUCT_GROUP_CODE, i) {
    const PRODUCT_GROUP_NAME$ = this.ProductGroup.find(
      (f) => f.PRODUCT_GROUP_CODE == PRODUCT_GROUP_CODE.value
    ).PRODUCT_GROUP_NAME;
  ...
  public DuplicatePorductGroup(PROD: any) {
    this.ProductGroup.map((m) => {
      const isSet = this.setProductGroupCode(PROD).filter(
        (f) => m.PRODUCT_GROUP_CODE == f.PRODUCT_GROUP_CODE
      ).length;
      if (isSet) m.IS_DISABLED = true;
      else m.IS_DISABLED = false;
    });
  }

  public setProductGroupCode(PROD: any[]) {
    let PRODUCT_GROUP_CODE: any[] = [];
    PRODUCT_GROUP_CODE = PROD.reduce(
      (a, c) => [...a, { PRODUCT_GROUP_CODE: c.PRODUCT_GROUP_CODE }],
      []
