  ...
  implements OnInit, OnDestroy
{
  ...
  async ngOnInit() {
    ...
    if (this.mode == "R") {
      ...
      await this.mainMasterService
        ...
        .then((res) => (this.typeheadStaff = res.RESPONSE_DATA || []));
    ...
    if (this.actionFrom == "edit") {
      ...
      if (res) {
        ...
        if (noticeDate) {
          ...
          res.NoticeDate =
            date.year + "-" + this.months[date.month - 1] + "-" + date.day;
        }
  ...
  async onclickSave() {
    ...
    const eDateCompare = getDateMyDatepicker(
      this.noticeForm.value.NOTICE_DUE_DATE
    );
    ...
    if (PRODUCT.length) {
      const PROD_NULL: boolean = PRODUCT.every(
        (e) => e.PRODUCT_GROUP_CODE != "" || null
      );
      if (!PROD_NULL) {
  ...
  async TransactionRunningHandling() {
    ...
    await this.transactionRunningService
      .TransactionRunninggetByCon_ph2("OPS_NOTICE", this.officeCode)
      .then(
        async (item) => {
          if (item.length == 0) {
            ...
            await this.transactionRunningService
              .TransactionRunningupdByCon_ph2(item[0].RUNNING_ID)
              .then(
                async (res) => {
                  if (res.IsSuccess) {
  ...
  clickDelete() {
    swal({
      ...
      if (result.value) {
        this.preloader.setShowPreloader(true);
        this.noticeService
          .NoticeupdDelete_Ph2(this.NOTICE_ID)
          .then((IsSuccess) => {
            this.preloader.setShowPreloader(false);
  ...
  private async setDataInit(res: any) {
    ...
    await res.NoticeLocale.map(
      (item) =>
        (item.Region = `${item.SubDistrict + "/" || ""}${
          item.District + "/" || ""
        }${item.Province}`)
    );
    ...
    suspect.map((item) => {
      item.SuspectFullName = !item.SuspectTitleName
        ? ""
        : item.SuspectTitleName;
      item.SuspectFullName += !item.SuspectFirstName
        ? ""
        : ` ${item.SuspectFirstName}`;
      item.SuspectFullName += !item.SuspectMiddleName
        ? ""
        : ` ${item.SuspectMiddleName}`;
      item.SuspectFullName += !item.SuspectLastName
        ? ""
        : ` ${item.SuspectLastName}`;

      item.CompanyFullName = !item.CompanyTitleName
        ? ""
        : item.CompanyTitleName;
      ...
      item.SuspectTypeName =
        item.SuspectTypeName ||
        this.suspectTypes.find((el) => parseInt(el.value) == item.SuspectType)
          .text;
      item.EntityTypeName =
        item.EntityTypeName ||
        this.entityTypes.find((el) => parseInt(el.value) == item.EntityType)
          .text;
    });
    ...
    product.map((item) => {
      ...
      item.DutyCode = this.typeheadProductUnit.find(
        (el) => parseInt(el.DutyUnitCode) == item.QtyUnit
      ).DutyCode;
    });
  ...
  private async getByCon(code: string) {
    ...
    await this.noticeService.getByCon(code).then(async (res) => {
      ...
      res.NoticeStaff.map((m) => {
        ...
        if (m.CONTRIBUTOR_ID == 7) {
          this.NoticeStaff.value[0] = m;
        }
        if (m.CONTRIBUTOR_ID == 8) {
          this.NoticeStaff.value[1] = m;
        }
        if (m.CONTRIBUTOR_ID == 9) {
          this.NoticeStaff.value[2] = m;
        }
        m.IsNewItem = false;
        m.IsUpdate = true;
      });
      ...
      res.NoticeSuspect.map((item) => {
        if (item.ENTITY_TYPE == "0") {
          item.ENTITY_TYPE_NAME = "บุคคลธรรมดา";
        } else if (item.ENTITY_TYPE == "1") {
          item.ENTITY_TYPE_NAME = "นิติบุคคล";
        }
        if (item.PERSON_TYPE == "0") {
          item.PERSON_TYPE_NAME = "คนไทย";
        } else if (item.PERSON_TYPE == "1") {
          item.PERSON_TYPE_NAME = "คนต่างชาติ";
        } else if (item.PERSON_TYPE == "2") {
          item.PERSON_TYPE_NAME = "ผู้ประกอบการ";
        }
        if (item.MISTREAT_NO == "0") {
          item.MISTREAT_NO_TEXT = "ไม่พบการกระทำผิด";
        } else {
          item.MISTREAT_NO_TEXT = item.MISTREAT_NO;
        }
        item.FULL_NAME = `${item.TITLE_SHORT_NAME_TH || ""}${
          item.FIRST_NAME || ""
        } ${item.LAST_NAME || ""}`;
      });

      let Locale: any = [];
      Locale = this.typeheadRegion.filter(
        (f) =>
          f.SUB_DISTRICT_ID ==
          this.noticeForm.value.NoticeLocale[0].SUB_DISTRICT_ID
      );
      if (Locale.length) {
        res.NoticeLocale.map((item, i) => {
          ...
          item.Region =
            Locale[i].SUB_DISTRICT_NAME_TH == null
              ? ""
              : `${Locale[i].SUB_DISTRICT_NAME_TH}`;
          item.Region +=
            Locale[i].DISTRICT_NAME_TH == null
              ? ""
              : ` ${Locale[i].DISTRICT_NAME_TH}`;
          item.Region +=
            Locale[i].PROVINCE_NAME_TH == null
              ? ""
              : ` ${Locale[i].PROVINCE_NAME_TH}`;
        });
      } else {
        res.NoticeLocale.map((item, i) => {
          item.Region = "";
      ...
      let LocInform: any = [];
      LocInform = this.typeheadRegion.filter(
        (f) =>
          f.SUB_DISTRICT_ID ==
          this.noticeForm.value.NoticeInformer[0].SUB_DISTRICT_ID
      );
      if (LocInform.length) {
        res.NoticeInformer.map((item, i) => {
          ...
          item.Region =
            LocInform[i].SUB_DISTRICT_NAME_TH == null
              ? ""
              : `${LocInform[i].SUB_DISTRICT_NAME_TH}`;
          item.Region +=
            LocInform[i].DISTRICT_NAME_TH == null
              ? ""
              : ` ${LocInform[i].DISTRICT_NAME_TH}`;
          item.Region +=
            LocInform[i].PROVINCE_NAME_TH == null
              ? ""
              : ` ${LocInform[i].PROVINCE_NAME_TH}`;
        });
      } else {
        res.NoticeInformer.map((item, i) => {
          item.Region = "";
    ...
    await this.noticeService
      .GetDocumentByCon(2, this.NOTICE_ID)
      .subscribe(async (res) => {
        this.fileList = [];
        let temp: any;
        temp = res;
  ...
  private async onCreate() {
    ...
    let t = (this.noticeForm.value.NOTICE_DATE =
      noticeDate.date.year +
      "-" +
      setZero(noticeDate.date.month) +
      "-" +
      noticeDate.date.day +
      " " +
      _noticeTime);
    this.noticeForm.value.NOTICE_DUE_DATE =
      noticeDueDate.date.year +
      "-" +
      setZero(noticeDueDate.date.month) +
      "-" +
      noticeDueDate.date.day +
      " " +
      _noticeDueTime;
    ...
    request = this.noticeService
      .NoticeinsAll(this.noticeForm.value)
      .pipe(
        mergeMap((x) => {
  ...
  private async onReviced() {
    ...
    const noticeDate = convertDateForSave(
      getDateMyDatepicker(this.noticeForm.value.NOTICE_DATE)
    );
    const noticeDueDate = convertDateForSave(
      getDateMyDatepicker(this.noticeForm.value.NOTICE_DUE_DATE)
    );

    const _noticeTime = `${this.noticeForm.value.NOTICE_TIME}`;
    const _noticeDueTime = `${this.noticeForm.value.NOTICE_DUE_TIME}`;

    this.noticeForm.value.NOTICE_DATE = this.toDateTZ(
      `${noticeDate} ${_noticeTime}`
    );
    this.noticeForm.value.NOTICE_DUE_DATE = this.toDateTZ(
      `${noticeDueDate} ${_noticeDueTime}`
    );
    this.noticeForm.value.UPDATE_DATE = dateTimeNow;

    this.noticeForm.value.UPDATE_USER_ACCOUNT_ID =
    ...
    request = this.noticeService
      .NoticeupdByCon(this.noticeForm.value)
      .pipe(
        mergeMap((x) => {
          return merge(
            Observable.of(x),
  ...
  NoticeDocumentModify(REFERENCE_CODE: string): Observable<any> {
    ...
    let ins$ = () =>
      from(ins).pipe(mergeMap((x) => this.noticeService.DocumentinsAll(x)));

    let del$ = () =>
      from(del).pipe(mergeMap((x) => this.noticeService.DocumentupdDelete(x)));
  ...
  NoticeProductModify(Product: any[]): Observable<any> {
    ...
    let ins$ = () =>
      from(ins).pipe(
        mergeMap((x) => this.noticeService.NoticeProductinsAll(x))
      );

    let upd$ = () =>
      from(upd).pipe(
        mergeMap((x) => this.noticeService.NoticeProductupdByCon(x))
      );

    let del$ = () =>
      from(del).pipe(
        mergeMap((x) => this.noticeService.NoticeProductupdDelete(x))
      );
  ...
  NoticeStaffModify(Staff: any[]): Observable<any> {
    ...
    let ins$ = () =>
      from(ins).pipe(mergeMap((x) => this.noticeService.NoticeStaffinsAll(x)));

    let upd$ = () =>
      from(upd).pipe(
        mergeMap((x) => this.noticeService.NoticeStaffupdByCon(x))
      );

    let del$ = () =>
      from(del).pipe(
        mergeMap((x) => this.noticeService.NoticeStaffupdDelete(x.STAFF_ID))
      );
  ...
  NoticeSuspectModify(Suspect: any[]): Observable<any> {
    ...
    let ins$ = () =>
      from(ins).pipe(mergeMap((x) => this.noticeService.NoticeSupectinsAll(x)));

    let del$ = () =>
      from(del).pipe(
        mergeMap((x) => this.noticeService.NoticeSuspectupdDelete(x.SUSPECT_ID))
      );
  ...
  addProduct() {
    this.modal = this.ngbModel.open(this.productsModel, {
      size: "lg",
      centered: true,
    });
  }
  ...
  addSuspect(suspect: any[]) {
    if (suspect.length) {
      ...
      let PERSON_ID = SUSP.reduce((a, c) => {
        return [...a, { PERSON_ID: c.PERSON_ID }];
      }, []);

      let pFilter = PERSON_ID.map((p) => {
        return p.PERSON_ID;
      });

      let filteredSuspect = suspect.filter(
        (suspect) => !pFilter.includes(suspect.PERSON_ID)
      );
      ...
      filteredSuspect.map((item) => {
        let noticeSuspect = {
          ...
          MISTREAT_NO_TEXT:
            item.MISTREAT_NO == 0 ? "ไม่พบการกระทำผิด" : item.MISTREAT_NO,
          IS_ACTIVE: 1,
          IsNewItem: true,
        };
  ...
  private async MasProductGroup() {
    await this.noticeService.MasProductGroupgetByCon("", "").then((res) => {
      ...
      if (this.ProductGroup.length) {
        this.ProductGroup = this.ProductGroup.reduce(
          (a, c) => [...a, { ...c, IS_DISABLED: false }],
          []
  ...
  public searchRegion = (text2$: Observable<string>) =>
    text2$
      ...
      .switchMap((term) =>
        this.noticeService
          .MasLocalegetByCon_Search({ TEXT_SEARCH: term })
  ...
  deleteStaff(i: number) {
    ...
    if (mStaffId)
      this.DelStaff.push(
        [noticeStaff].reduce((acc, curr) => {
          return { STAFF_ID: curr.STAFF_ID };
        }, {})
      );
  ...
  blurDataUnit(ele: any, index: number) {
    ...
    if (!ele.value) {
      ...
      let units = this.typeheadProductUnit
        .filter(
          (v) => v.DutyCode.toLowerCase().indexOf(text.toLowerCase()) > -1
        )
        .slice(0, 10);
  ...
  async onDeleteProduct() {
    if (this.mode === "C") this.NoticeProduct.removeAt(this.productIndex);
    else if (this.mode === "R") {
  ...
  beforeDeleteSuspect(id: string, index: number) {
    ...
    swal({
      ...
      if (result.value) {
        if (this.mode === "C") this.NoticeSuspect.removeAt(index);
        else if (this.mode === "R") {
          this.DelSuspect.push(
            [id].reduce((acc, curr) => {
              return { SUSPECT_ID: curr };
            }, {})
          );
  ...
  openSuspect(e) {
    this.modal = this.suspectModalService.open(e, {
      size: "lg",
      centered: true,
    });
  }
  ...
  async onCancelConfirmClick() {
    swal({
      ...
      if (result.value) {
        if (this.mode === "R") this.onComplete();
        else this.onCancelEdit();
      }
    });
  }
  ...
  selecterSuspectView(Mode, MISTREAT_NO) {
    if (MISTREAT_NO == 0 || Mode == "C")
      setTimeout(() => {
        this.modal = this.ngbModel.open(this.CreateSuspectModal, {
          windowClass: "my-class",
        });
      }, 200);
    else
      setTimeout(() => {
        this.modal = this.ngbModel.open(this.ViewLawbreakerModal, {
          size: <any>"lg",
          centered: true,
        });
      }, 200);
