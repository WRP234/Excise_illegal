  ...
  implements OnInit, OnDestroy, DoCheck
{
  ...
  myDatePickerArrestFromOption: IMyOptions = {
  ...
  myDatePickerArrestToOption: IMyOptions = {
    editableDateField: false,
    dateFormat: "dd mmm yyyy",
    showClearDateBtn: true,
    height: "30px",
  ...
  async ngOnInit() {
    ...
    this.dateToOption.disableDateRanges = [
      this.getDisLessthanCurrDateMyDatePicker(),
  ...
  onAdvSearch(form: any) {
    ...
    f.OCCURRENCE_DATE_TO =
      convertDateForSave(edate) || convertDateForSave(new Date());
  ...
  private onSearchComplete(list: ArrestList[]) {
    ...
    this.arrest = this.arrest.map((x, i) => {
      x.SUBSECTION_NAME$ = x.ArrestMasGuiltbase.reduce(
        (a, c, i) => [...a, `${this.comma(i)}${c.SUBSECTION_NAME}`],
        ""
      );
      x.GUILTBASE_NAME$ = x.ArrestMasGuiltbase.reduce(
        (a, c, i) => [...a, `${this.comma(i)}${c.GUILTBASE_NAME}`],
        ""
      );
  ...
  clickNew = () => this.router.navigate([`/arrest/manage/C/NEW`]);

//   public onSDateChange(event) {
//     if (event.jsdate) {
...
//   }
  ...
  public onSDateChange(event) {
    if (event.jsdate) {
      var d = new Date(event.jsdate);
      d.setDate(d.getDate() - 1);
      this.myDatePickerArrestToOption = {
        disableDateRanges: [
          {
            begin: {
              year: d.getFullYear() - 100,
