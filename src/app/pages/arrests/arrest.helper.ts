...
export class ArrestHelper {
  ...
  public yy_th = (new Date().getFullYear() + 543).toString().substr(2, 2);
  ...
  public setDateStruct(date) {
    if (date) {
      ...
      const CompDate = `${temp.getUTCDate()} ${months[temp.getMonth()]} ${
        temp.getUTCFullYear() + 543
      }`;
      return CompDate;
    } else return null;
  }
  ...
  public setDateForThaiYear(date) {
    const months = this.I18N_VALUES.months;
    const temp = (date = new Date(date));
    const CompDate = `${temp.getUTCDate()} ${
  ...
  setPersonReferenceNo(item: ArrestLawbreaker): string {
    ...
    switch (item.PERSON_TYPE) {
      ...
      case 1: // ต่างชาติ
        ...
        break;
      case 2: // นิติบุคคล
        if (item.ENTITY_TYPE == 1) REFERENCE_NO = item.COMPANY_REGISTRATION_NO;
        else if (item.ENTITY_TYPE == 0) REFERENCE_NO = item.ID_CARD;
        break;
    }
    return REFERENCE_NO;
  }
  ...
  ConvDateTimeToDate = (datetime: string) =>
  ...
  T_LocaleFormatter = (x: {
    ...
    `${x.SUB_DISTRICT_NAME_TH || ""} ${x.DISTRICT_NAME_TH || ""} ${
      x.PROVINCE_NAME_TH || ""
    }`;

  T_MasProductGroupFormatter = (x: { PRODUCT_GROUP_NAME: string }) =>
    x.PRODUCT_GROUP_NAME;

  T_MasProductCategory = (x: { PRODUCT_CATEGORY_NAME: string }) =>
    x.PRODUCT_CATEGORY_NAME;

  T_MasProductType = (x: { PRODUCT_TYPE_NAME: string }) => x.PRODUCT_TYPE_NAME;

  T_MasProductSubType = (x: { PRODUCT_SUBTYPE_NAME: string }) =>
    x.PRODUCT_SUBTYPE_NAME;

  T_MasProductSubSetType = (x: { PRODUCT_SUBSETTYPE_NAME: string }) =>
    x.PRODUCT_SUBSETTYPE_NAME;

  T_MasProductBrand = (x: { PRODUCT_BRAND_NAME_TH: string }) =>
    x.PRODUCT_BRAND_NAME_TH;

  T_MasProductSubBrand = (x: { PRODUCT_SUBBRAND_NAME_TH: string }) =>
    x.PRODUCT_SUBBRAND_NAME_TH;

  T_MasProductModel = (x: { PRODUCT_MODEL_NAME_TH: string }) =>
    x.PRODUCT_MODEL_NAME_TH;

  T_MasCountry = (x: { COUNTRY_NAME_TH: string }) => x.COUNTRY_NAME_TH;

  T_MasTitle = (x: { TITLE_NAME_TH: string }) => x.TITLE_NAME_TH;

  T_MasTitleShort = (x: { TITLE_SHORT_NAME_TH: string }) =>
    x.TITLE_SHORT_NAME_TH;

  T_MasTitleShortEN = (x: { TITLE_SHORT_NAME_EN: string }) =>
    x.TITLE_SHORT_NAME_EN;

  T_MasRace = (x: { RACE_NAME_TH: string }) => x.RACE_NAME_TH;

  T_MasNationality = (x: { NATIONALITY_NAME_TH: string }) =>
    x.NATIONALITY_NAME_TH;
...
export function IsEmpty<T>(
  thing: T | undefined | null | 0 | false | "" | "null"
): thing is Empty {
...
export function sortingArray(arr: any[], arg: string) {
  ...
  let i = 0;
  a.map((x) => {
    if (x[arg] < IntialLastRowID) x[arg] = ++i;
  });
  return a;
}
